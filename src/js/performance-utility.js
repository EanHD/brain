...
export async function measureOperation(operation, fn) {
  ...
  try {
    ...
    const measurement = timer.end();

    // Attach measurement to rich results without changing return contract
    if (result && typeof result === 'object') {
      Object.defineProperty(result, Symbol.for('performanceMeasurement'), {
        value: measurement,
        enumerable: false,
        writable: false
      });
