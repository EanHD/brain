...
export class AIService {
  ...
  async _loadSettings() {
    try {
      ...
      if (!this.apiKey && !this.isPrivateMode) {
        console.log('ℹ️ AI API key not configured. AI features will be disabled until key is provided.');
        this.eventBus.emit(APPLICATION_EVENTS.AI_CONFIGURATION_REQUIRED);
      } else if (this.apiKey) {
        console.log('✅ AI service loaded with existing API key');
      }
    } catch (error) {
  ...
  await db.setting('ai_api_key', this.apiKey, { isEncrypted: true });
...
Example response: ["javascript", "web-development", "arrays", "tutorial"]`;
  ...
  async generateCompletion(messages, options = {}) {
    return await measureOperation('ai-request', async () => {
      const { force = false, maxTokens = 1000, temperature = 0.7 } = options;

      // Check private mode
      if (this.isPrivateMode && !force) {
        throw new Error('AI completions are disabled in private mode');
      }

      // Validate input
      if (!Array.isArray(messages) || messages.length === 0) {
        throw new Error('Messages array is required for completion generation');
      }
      ...
      try {
        ...
        try {
          const response = await fetch(`${AI_CONFIG.API_ENDPOINT}/chat/completions`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${this.apiKey}`
        ...
        console.error('AI completion generation failed:', error);
