...
function encodeBase32(value, length) {
  ...
  // Ensure fixed length by padding with leading zeros if necessary
...
export function generateULID(timestamp = Date.now()) {
  ...
  // Normalize length
  if (randomStr.length > RANDOM_LENGTH) {
    randomStr = randomStr.slice(0, RANDOM_LENGTH);
  }
 ...
 */
function isValidStructure(ulid) {
  if (typeof ulid !== 'string' || ulid.length !== TIME_LENGTH + RANDOM_LENGTH) {
    return false;
  }
...
export function isValidULID(ulid) {
  if (!isValidStructure(ulid)) {
    return false;
  }

  try {
    const timestamp = decodeBase32(ulid.substring(0, TIME_LENGTH));
...
export function extractTimestamp(ulid) {
  ...
  const timestamp = decodeBase32(timeStr);
...
export function generateMonotonicULID(timestamp = Date.now()) {
  if (timestamp === lastTime && lastRandom !== null) {
    ...
    // New millisecond or first call - reset random seed
    lastTime = timestamp;
    lastRandom = randomBytes();
  }
