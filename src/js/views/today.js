...
export class TodayViewController {
  ...
  handleNoteInput() {
    ...
    // Expand/collapse note input
    const noteInputContainer = this.elements.noteInput.closest('.note-input-container');
    if (noteInputContainer) {
      noteInputContainer.classList.toggle('expanded', hasContent);
    }
  }
  ...
  handleClearInput() {
    this.elements.noteInput.value = '';
    this.elements.noteInput.style.height = '60px';
  ...
  updateCharacterCount() {
    ...
    if (charCountEl) {
      const count = this.elements.noteInput.value.length;
  ...
  renderRecentNotes(notes) {
    if (notes.length === 0) {
      ...
      const emptyState = NoteCard.createEmptyState(
        'No notes yet',
        {
          label: 'Start Writing Above',
          onClick: () => this.elements.noteInput.focus()
        }
      );
      this.elements.todayNotes.appendChild(emptyState);
      return;
    }
    ...
    notes.forEach((note, index) => {
      const card = NoteCard.create(note, {
        showActions: true,
        showTags: true,
        showMeta: true,
    ...
    this.elements.todayNotes.appendChild(fragment);
  }
